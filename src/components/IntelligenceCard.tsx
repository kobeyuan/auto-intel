...
export function IntelligenceCard({ item }: IntelligenceCardProps) {
  ...
  return (
    <div
      className={`group border-b border-slate-100 hover:bg-blue-50/30 transition-all cursor-pointer ${isExpanded ? 'bg-blue-50/50' : ''}`}
      onClick={() => setIsExpanded(!isExpanded)}
    >
      <div className="flex items-center gap-4 px-8 py-4 min-h-[60px]">
        <div className={`flex-shrink-0 w-12 h-6 flex items-center justify-center rounded-lg text-[10px] font-black border ${getScoreColor(item.quality_score || 0)}`}>
          {item.quality_score?.toFixed(1)}
        </div>

        <div className="flex-shrink-0 flex gap-2">
          {tags.slice(0, 2).map(tag => (
            <span key={tag} className="text-[9px] font-bold text-slate-400 uppercase tracking-tight border border-slate-200 px-2 py-0.5 rounded-md bg-white">
              {tag}
            </span>
          ))}
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm space-y-6">
            <div className="flex items-start justify-between gap-6">
              ...
              <a
                ...
                className="p-2.5 bg-slate-50 hover:bg-blue-600 hover:text-white rounded-xl transition-all shadow-sm"
              >
            ...
            <div className="pt-2">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Raw Intelligence Context</span>
              </div>
              <p className="text-sm leading-relaxed text-slate-500 font-medium">
                系统已对该公关稿进行脱水处理。原始信息聚焦于 <span className="text-slate-800 font-bold">{item.title}</span>。
